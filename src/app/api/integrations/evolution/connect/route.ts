...
import { encryptIntegrationConfig, isIntegrationEncryptionConfigured } from '@/lib/integration-secrets';
...
export async function POST() {
  try {
    ...
    if (!isIntegrationEncryptionConfigured()) return NextResponse.json({ error: 'مفتاح تشفير التكاملات INTEGRATION_ENCRYPTION_KEY غير مضبوط أو غير صالح في Vercel.' }, { status: 503 });
    ...
    if (!response.ok) return NextResponse.json({ error: `تعذر إنشاء جلسة واتساب في Evolution API (HTTP ${response.status}). تحقق من الرابط والمفتاح.` }, { status: 502 });
    ...
    console.error('Evolution connect failed', error);
    return NextResponse.json({ error: error instanceof Error && error.message.includes('fetch failed') ? 'تعذر الوصول إلى خادم Evolution API. تحقق من الرابط وSSL.' : 'حدث خطأ خادمي أثناء إنشاء جلسة واتساب.' }, { status: 500 });
  }
}
